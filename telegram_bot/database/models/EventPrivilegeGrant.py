...
class EventPrivilegeGrant(Base):
	...
	privileges: Mapped[int] = mapped_column(postgresql.ENUM(EventPrivilege), nullable=False, comment="флаги привилегий")
	...
	# Кто ответственный
	responsible_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), nullable=False, comment="кому предназначено")
	responsible: Mapped['User'] = relationship('User', back_populates='event_privileges', foreign_keys=[responsible_id])
