...
async def get_category_by_name(session: AsyncSession, name : str) -> MerchCategory | None:
	query = (
		select(MerchCategory)
...
async def get_category_by_id(session: AsyncSession, id : int) -> MerchCategory | None:
	query = (
		select(MerchCategory)
		.where(MerchCategory.id == id)
		.options(selectinload(MerchCategory.merch_items))
	)
	result = await session.execute(query)
	return result.scalars().first()
