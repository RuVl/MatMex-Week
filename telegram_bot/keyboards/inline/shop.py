...
from keyboards.callback_factories import (
	...
	ShopCategoryFactory,
	ShopDeleteCategoryFactory,
	ShopDeleteItemFactory,
	ShopItemFactory,
)
	...
	for category in categories:
		builder.row(
			InlineKeyboardButton(
				text=category.name,
				callback_data=ShopCategoryFactory(category_id=category.id, can_delete=can_delete).pack(),
			),
		)
	return builder.as_markup(
		resize_keyboard=True,
		input_field_placeholder=l10n.format_value('placeholder-category'),
	)
...
async def get_edit_shop_category_ikb(l10n) -> InlineKeyboardMarkup:
	...
	for category in categories:
		builder.row(
			InlineKeyboardButton(
				text=category.name,
				callback_data=EditShopCategoryFactory(category_id=category.id).pack(),
			),
		)
	builder.row(
...
def get_item_ikb(l10n, category: MerchCategory, can_delete: bool) -> InlineKeyboardMarkup:
	...
	return builder.as_markup(
		resize_keyboard=True,
		input_field_placeholder=l10n.format_value('placeholder-item'),
	)


def item_actions_ikb(l10n, item: MerchItem, can_delete: bool) -> InlineKeyboardMarkup:
	builder = InlineKeyboardBuilder()
	...
	if can_delete:
		...
		builder.row(
			InlineKeyboardButton(
				text=l10n.format_value('btn-buy'),
				callback_data=IsSureBuyItemFactory(item_id=item.id, category_id=item.category_id, can_delete=can_delete).pack(),
			)
		)
	builder.row(
		InlineKeyboardButton(
	...
	return builder.as_markup(
		resize_keyboard=True,
		input_field_placeholder=l10n.format_value('placeholder-get-back-to-item'),
	)


def yes_no_buy_ikb(l10n, item: MerchItem, can_delete: bool) -> InlineKeyboardMarkup:
	builder = InlineKeyboardBuilder()
	builder.row(
		InlineKeyboardButton(
			text=l10n.format_value('btn-yes'),
			callback_data=BuyItemFactory(item_id=item.id, category_id=item.category_id, can_delete=can_delete).pack(),
		),
		InlineKeyboardButton(
			text=l10n.format_value('btn-no'),
			callback_data=ShopItemFactory(item_id=item.id, category_id=item.category_id, can_delete=can_delete).pack(),
		),
	)
	return builder.as_markup(
		resize_keyboard=True,
...
def get_item_size_ikb(l10n) -> InlineKeyboardMarkup:
	...
	return builder.as_markup(
		resize_keyboard=True,
		input_field_placeholder=l10n.format_value('placeholder-item-size'),
	)
