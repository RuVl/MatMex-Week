...
async def grant_privileges_h(msg: types.Message, state: FSMContext, l10n: FluentLocalization):
...
async def user_choice_h(callback: types.CallbackQuery, callback_data: UserFactory, state: FSMContext, l10n: FluentLocalization):
	...
	async with async_session() as session:
		subject = await get_user_by_telegram_id(session, subject_telegram_id)
	...
	async with async_session() as session:
		...
		if not subject_privileges:
			subject_privileges = await create_privilege(
				session=session,
				user_id=subject.id,
				privilege_mask=0,
				provider_id=admin_privileges.id
			)
...
async def privileges_kb_h(callback: types.CallbackQuery, callback_data: PrivilegeButtonFactory, l10n: FluentLocalization):
	...
	if not can_grant:
		await callback.answer(l10n.format_value("cant-change-privileges"))
		return
	...
	await callback.message.edit_reply_markup(
		reply_markup=user_rights_ikb(
			l10n,
			admin_privileges.privilege,
			subject_privileges.privilege,
